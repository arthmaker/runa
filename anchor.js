...
function extractAdaptiveKeywords(title, minWords = 3, maxWords = 3){
  ...
  const isWordAllowed = (word, minLength = 4) => word.length >= minLength && !STOPWORDS_ID.has(word);

  let filtered = titleTokens.filter((word) => isWordAllowed(word, 4));
  if(filtered.length === 0){
    filtered = titleTokens.filter((word) => isWordAllowed(word, 2));
  }
  const unique = [];
  ...
  const fallback = titleTokens
    .filter((word) => isWordAllowed(word, 2))
